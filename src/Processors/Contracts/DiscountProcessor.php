...
abstract class DiscountProcessor
{
     ...
     */
    final public function setOrderId(int|null $orderId): static
    {
        $this->orderId = $orderId;
    ...
    /**
     * Tabulate the discount device/device-line for a discountable.
     *
     * @param  DiscountInstrument  $discountInstrument
     * @return void
     */
    protected function scan(DiscountInstrument $discountInstrument, DiscountableDevice $discountableDevice): void
    {
        ...
        // the discount instrument is applied to all discountable devices. Otherwise
        // the discount instrument is applied to discountable devices' discountable
        // lines with an applicable discountable.
        if (empty($instrumentDiscountables)) {
            // Apply discount to discountable devices.
            if ($discountableDevice->subtotal() < $discountInstrument->getRestrictionsMinimumAmount()) {
                return;
            }
            $this->apply($discountInstrument, $discountableDevice);
        } else {
            ...
            foreach ($instrumentDiscountables as $instrumentDiscountable) {
                $discountableDeviceLines = array_merge(
                    $discountableDeviceLines,
                    $discountableDevice->getDiscountableDeviceLines($instrumentDiscountable)
                );
            }

            // Check the minimum quantity criteria. Note that this is specific to the
            // discount instrument with discountables. Additionally there is no
            // minimum amount check in this case.
            if (count($discountableDeviceLines) < $discountInstrument->getMinQuantity()) {
                return;
            }

            // Apply discount in chunks of the unit quantity
            $discountableDeviceLinesChunks = array_chunk($discountableDeviceLines, $discountInstrument->getUnitQuantity());

            $unit_quantities = count($discountableDeviceLinesChunks);
            $counter = 0;
            foreach ($discountableDeviceLinesChunks as $discountableDeviceLinesChunk) {
                if (count($discountableDeviceLinesChunk) < $discountInstrument->getUnitQuantity()) {
                    continue;
                }
                $unit_quantity_group = $unit_quantities.'_'.$counter;
    ...
    protected function apply(DiscountInstrument $discountInstrument, DiscountableDevice $discountableDevice, array $discountableDeviceLines = [], string $unit_quantity_group = 'none')
    {
        ...
        if (empty($discountableDeviceLines)) { // Apply the discount to the discountableDevice.
            $discountLine = $this->discountLineList->getOrAdd($discountableDevice);
            $subtotal_remainder = $discountLine->remainder();
            $discount_amount = $this->off($subtotal_remainder, $discountInstrument);

            $discountLines = [$discountLine];
            $discount_shares = [$discountLine->getHash() => $discount_amount];
        } else { // Apply the discount to the discountableDeviceLines as a unit/whole.
            switch($discountInstrument->getDiscountType()) {
                ...
                case DiscountTypes::PERCENT_OFF:
                    ...
                    foreach ($discountableDeviceLines as $discountableDeviceLine) {
                        $discountLine = $this->discountLineList->getOrAdd($discountableDevice, $discountableDeviceLine);
                        $subtotal_remainder += $discountLine->remainder();
                        $discountLines[] = $discountLine;
                    }
                    $discount_amount = $this->off($subtotal_remainder, $discountInstrument);
     ...
     * @return array
     */
    private function share(int $amount, int $count): array
    {
     ...
     */
    private function tricklingShare(string $strategy, int $amount, DiscountLine ...$discountLines): array
    {
        $distributor = new AmountDistributor();
    ...
    /**
     * Determine the actual amount that will be discounted
     *
     * @param  int  $subtotal
     * @param  DiscountInstrument  $discountInstrument
     * @return int
